...
class ComputerPlayer(Player):
    ...
    def getMax(self, otherPlayerSym, possibleMoves_cols,depth, isPruning, alpha, beta,nodes_expanded):
        # @ returns the maximum possible move
        #print("depth(max): " + str(depth))
        maxCol_value = (float('-inf'), None,nodes_expanded)
        depth += 1
        for boardState,col in possibleMoves_cols:
            ...
            move=self.getMove(otherPlayerSym, boardState,depth, isPruning, alpha, beta,nodes_expanded)
            nodes_expanded=move[2]+1
            temp_value = max(maxCol_value[0], move[0])
            if(isPruning):
                alpha = max(alpha, temp_value)
            if maxCol_value[0] < temp_value:
                maxCol_value = temp_value,col,nodes_expanded
        return maxCol_value  


    def getMin(self, otherPlayerSym, possibleMoves_cols,depth, isPruning, alpha, beta,nodes_expanded):
        # @ returns the minimum possible move
        #print("depth(min): " + str(depth))
        minCol_value = (float('inf'), None,nodes_expanded)
        depth += 1
        for boardState,col in possibleMoves_cols:
            if(isPruning and alpha >= beta):
                #print("pruned")
            ...
            move=self.getMove(otherPlayerSym, boardState,depth, isPruning, alpha, beta,nodes_expanded)
            nodes_expanded=move[2]+1
            temp_value = min(minCol_value[0], move[0])
            if(isPruning):
              beta = min(temp_value, beta)
            if minCol_value[0] > temp_value:
                minCol_value = temp_value,col,nodes_expanded
        return minCol_value

    def getMove(self, playerSym, boardState,depth, isPruning, alpha=float('-inf'), beta=float('inf'),nodes_expanded=1):
        ##print(depth)
        if(isGameFull(boardState)):
            return numOf4Connected(playerSym, boardState),None,nodes_expanded
        if(depth >= 2):
        ...
        if(playerSym == self.sym):
            return self.getMax(self.sym, possibleMoves_cols, depth, isPruning,alpha, beta,nodes_expanded)
        else:
            return self.getMin(self.otherSym, possibleMoves_cols,depth, isPruning, alpha, beta,nodes_expanded)
